...
func NewServerRoute(logger *zap.SugaredLogger,
	loggerRaw *zap.Logger,
	config *config.Configuration) http.Handler {
	r := chi.NewRouter()
	...
	r.Get("/metrics/all", handler.GetAllMetrics)
	r.Post("/metrics/filter", handler.GetMetricsValues)
	return r
}
